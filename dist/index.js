...
var Orders = class {
  ...
  parsePlaceRequest(request2) {
    const payload = {
      ...request2,
      correlationId: request2.correlationId ?? randomUUID(),
...
var superOrderSchema = z2.object({
  dhanClientId: z2.string().min(1).optional(),
  correlationId: z2.string().min(1).max(128).optional(),
  transactionType: z2.enum(["BUY", "SELL"]),
  exchangeSegment: z2.enum([
    "NSE_EQ",
    "NSE_FNO",
    "NSE_COMM",
    "BSE_EQ",
  ...
  productType: z2.enum(["CNC", "INTRADAY", "MARGIN", "MTF"]),
  orderType: z2.enum(["MARKET", "LIMIT"]),
  quantity: z2.number().int().positive(),
  price: z2.number().nonnegative().optional(),
  targetPrice: z2.number().nonnegative().optional(),
  stopLossPrice: z2.number().nonnegative().optional(),
  trailingJump: z2.number().nonnegative().optional(),
  securityId: z2.string().min(1)
}).superRefine((value, ctx) => {
...
var SuperOrders = class {
  ...
  parsePlaceRequest(request2) {
    const payload = {
      ...request2,
      correlationId: request2.correlationId ?? randomUUID2(),
      dhanClientId: request2.dhanClientId ?? this.httpClient.getClientId()
    };
    try {
...
export {
  ...
  Positions,
  Profile,
  promptHelpers_exports as PromptHelpers,
  ProtectivePutSkill,
  READ_SCOPES,
  RESOURCES,
  RateLimitError,
  ...
  TechnicalAnalysis,
  TokenManager,
  schemas_exports as ToolSchemas,
  TraderControls,
  TrailManager,
  UnknownMethodError,
  ValidationError,
