...
var HttpClient = class {
  constructor(config, dependencies = {}) {
    ...
    this.axiosInstance = dependencies.axiosInstance ?? import_axios2.default.create({
  ...
  normalizeError(error) {
    if (error instanceof Error && !(error instanceof import_axios2.AxiosError)) {
      return error;
    }
    if (error instanceof import_axios2.AxiosError || isAxiosLikeError(error)) {
      const axiosError = error;
      if (axiosError.response) {
...
var Orders = class {
  ...
  parsePlaceRequest(request2) {
    const payload = {
      ...request2,
      correlationId: request2.correlationId ?? (0, import_crypto.randomUUID)(),
...
var superOrderSchema = import_zod3.z.object({
  dhanClientId: import_zod3.z.string().min(1).optional(),
  correlationId: import_zod3.z.string().min(1).max(128).optional(),
  transactionType: import_zod3.z.enum(["BUY", "SELL"]),
  exchangeSegment: import_zod3.z.enum([
    "NSE_EQ",
    "NSE_FNO",
    "NSE_COMM",
    "BSE_EQ",
  ...
  productType: import_zod3.z.enum(["CNC", "INTRADAY", "MARGIN", "MTF"]),
  orderType: import_zod3.z.enum(["MARKET", "LIMIT"]),
  quantity: import_zod3.z.number().int().positive(),
  price: import_zod3.z.number().nonnegative().optional(),
  targetPrice: import_zod3.z.number().nonnegative().optional(),
  stopLossPrice: import_zod3.z.number().nonnegative().optional(),
  trailingJump: import_zod3.z.number().nonnegative().optional(),
  securityId: import_zod3.z.string().min(1)
}).superRefine((value, ctx) => {
